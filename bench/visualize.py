...
def plot_sorting_algorithms_time_comparison(data, output_path):
    """
    Plots a comparison of sorting algorithms" mean execution times across various input types
    and saves the plot as a PNG file.

    Parameters:
    - data (dict): The input data containing sorting algorithms" execution times for different input types.
    ...
    # Create a unique list of input types (e.g., "random", "ordered", etc.)
    ...
    # Create a colormap
    cmap = cm.viridis  # You can replace this with any other colormap, like 'plasma', 'inferno', etc.

    width = 0.215  # Width of the bars
    x_positions = np.arange(len(unique_names))  # X positions for input types

    # A dictionary to hold the "fast" algorithm's mean time for each input type
    fast_algorithm_times = {}
        ...
        for name in unique_names:
            ...
            for i in range(len(names)):
                if names[i] == name and kinds[i] == kind:
                    ...
                    found = True
                    if kind == "fast":  # Save the "fast" algorithm time for percentage calculation
                        fast_algorithm_times[name] = time_mean[i]
                    break
            if not found:
        ...
        for i, bar in enumerate(bars):
            if kind != "fast":  # Skip annotation for "fast" bars
                ...
                if whisker_top + approx_text_height < y_max:
                    ...
                    ax.annotate(f'{pct_deviation:+.1f}%', 
    ...
    ax.set_xticklabels(unique_names, rotation=45, ha='right', fontsize=12)
...
def process_reports(input_folder):
    """
